...
export default function AIPetGenerator() {
  ...
  const handleGenerate = async (preferencesData: any) => {
    setPreferences(preferencesData);
    setHasGenerated(false);
    console.log("Generating AI pet with:", preferencesData);
    setLoading(true);
    setError("");

    try {
      const queryParams = new URLSearchParams({
        pet_size: preferencesData.pet_size,
        physique: preferencesData.physique,
        pet_type: preferencesData.pet_type,
        pattern: preferencesData.pattern,
        fur: preferencesData.fur,
                ...
                {searchingMatches ? (
                  ...
                  <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-6">
                    {strays
                      .filter(
                        (stray) =>
                          !preferences || stray.species_type === preferences.pet_type
                      )
                      .map((stray, index) => (
